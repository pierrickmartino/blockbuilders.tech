...
- Attempted `python3 -m pytest apps/api/tests` (fails: `ModuleNotFoundError: No module named 'pytest'` in sandbox without dependency install)
- Attempted `pnpm --filter web test` (fails: `vitest: command not found` because workspace dependencies are not installed in sandbox)
- Attempted `pnpm --filter web lint` (fails: `next: command not found` because workspace Node dependencies are unavailable in sandbox)
- Attempted `poetry run pytest` (fails: virtualenv destination under `~/Library/Caches/pypoetry/virtualenvs` is not writable in sandbox)
...
- Tests not executed locally due to unavailable Python/Node dependencies in sandbox; new suites added for FastAPI routes and Zustand store.

...
- [x] Verification and automated testing coverage (AC: 1-4)  
  ...
  - [x] Use the local Datadog shim by setting `DATADOG_LOG_ENDPOINT=http://127.0.0.1:8282/logs` during integration tests (patch `settings.datadog_log_endpoint`) and assert payloads with `tests/utils/datadog_sink.py` so telemetry expectations are verifiable offline.  
...
- Ran `cd apps/api && poetry run pytest tests/test_supabase_service.py` (pass; exercises Supabase fallback/cache logic for offline consent persistence).

### Completion Notes List
- Added Supabase admin fallback that decodes JWT metadata, caches consent locally, and respects a configurable `SUPABASE_HTTP_TIMEOUT_SECONDS` (set to `0` to disable remote calls) so sign-in flows stay responsive when the external consent endpoint times out, with targeted pytest coverage guarding the regression.
...
- apps/web/tests/login-consent.test.tsx
- apps/web/tests/onboarding-callouts.test.ts

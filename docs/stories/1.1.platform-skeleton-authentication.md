...
- [ ] Instrument audit logging for auth events and workspace creation (AC: 4)  
  ...
  - [ ] Add Datadog log-based monitor and dashboard tiles (auth success/fail counts, workspace creation events) to verify telemetry lands in the expected `service:auth` and `service:workspace` indexes ([Source: architecture/security-and-performance.md#observability])  
  - [ ] Capture compliance ingestion by writing `ComplianceEvent` rows and exporting a sample audit CSV to confirm storage schema matches governance requirements ([Source: architecture/data-models.md#complianceevent], [Source: architecture/deployment-architecture.md#seed-data-governance])

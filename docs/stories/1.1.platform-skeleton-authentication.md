...
- [x] Provision demo workspace after authentication (AC: 3)  
  ...
  - [ ] Attach the onboarding callouts `welcome-consent`, `canvas-tour`, `edit-parameters`, and `run-first-backtest` to the dashboard redirect so the checklist and tooltip system reinforce AC3. ([Source: docs/front-end-spec.md#onboarding-checklist])
...
- [ ] Verification and automated testing coverage (AC: 1-4)  
  ...
  - [ ] Create `tests/utils/datadog_sink.py` to accept log payloads from the local Datadog shim, persist them for assertions, and expose helpers used by backend and E2E suites.  
...
- Ran `pnpm --filter web lint` (pass after adding explicit `role="tab"` to authentication mode toggles).
- Ran `pnpm --filter web test` (pass; Vitest suite green).
- Attempted to provision FastAPI test environment via `poetry install`/`pip install`; blocked by sandboxed environment lacking network access to fetch Python packages, so pytest suite could not be executed.
- Ran `pnpm --filter web lint` (pass; verifies consent middleware guard does not introduce lint regressions).
- Ran `pnpm --filter web test` (pass; Vitest suite remains green after middleware guard).

### Completion Notes List
- Turborepo monorepo scaffolded with Next.js 15 front-end, FastAPI gateway, Celery workers, and shared TypeScript/Pydantic contracts plus CI workflow and env templates.
...
- Frontend lint/tests executed successfully; FastAPI pytest suite still pending due to restricted network preventing dependency installation inside sandbox.
